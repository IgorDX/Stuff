...
export const ShowcaseCategory : React.FC<ShowcaseCategoryProps> = ({
  ...
  return (
    <div className='group hover:shadow-customPink transition-shadow duration-300 relative card flex flex-col rounded-b-lg bg-heroGray flex-shrink-0'>
        ...
        <img className="rounded-t-md object-cover xl:w-[234px] xl:h-[234px] h-full w-full"  src={category.image} alt="Category image" />
        <h3 className='flex items-center justify-center text-white font-semibold leading-4 py-3 text-sm max-w-[200px] mb-3'>{category.name}</h3>
    </div>
  )
}
