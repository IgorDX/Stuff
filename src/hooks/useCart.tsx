...
export const useCart = ({ animationDelay = 300 }: UseCartProps) => {
  ...
  const onDecreaseQuantity = useCallback((id: number, quantity: number) => {
    if (quantity > 1) {
  ...
  const onIncreaseQuantity = useCallback((id: number) => {
    dispatch(increaseQuantity({ id }));
  }, [dispatch]);

  const onRemoveFromCart = useCallback((id: number) => {
    dispatch(removeFromCart({ id }));
  }, [dispatch]);

  const onRemoveFromCartDelayed = useCallback((id: number) => {
    setIsRemoved(true);
