...
export const MobileNav: React.FC<MobileNavProps> = ({ isOpen, setIsOpen }) => {
  const { userInputDrawerOpen, setUserInputDrawerOpen, handleClose } = useMobileNav(isOpen, setIsOpen);
  ...
  return (
    <>
      ...
      <div
        className={`fixed w-full h-full transition-all transform duration-300 bg-heroGray z-[100] pr-5 sm:pr-12 ${isOpen ? '-translate-y-3 opacity-100' : '-translate-y-1/4 opacity-0 invisible'}`}
      >
        <div className="mb-3">
          <UserInput
