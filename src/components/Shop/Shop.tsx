...
export const Shop = () => {
  ...
  const {
    ...
    localPrice,
    handlePriceChange, 
    handleSearchSubmit} = useShopSearch(title, categoryId, price, searchUI.handleBlur, searchUI.handleFocus);
  ...
  <div className="w-full xs:max-w-[230px] relative z-50">
    <ShopSearch searchUI={searchUI} handleTitleChange={handleTitleChange} localTitle={localTitle} ></ShopSearch>
  </div>
  <input
    value={localPrice}
  ...
  />
   <button type="submit" style={{ display: 'none' }} />
            ...
            { showLoading ? <Loading></Loading>
              : data.length ? <div className={`transition-all duration-700 ${fadeIn ? 'opacity-100' : 'opacity-0 translate-y-12'} `}>
