...
export const createUser = createAsyncThunk<User, UserSignUpFormValues>('users/createUsers', async (payload, thunkAPI) =>{
  const res = await axios.post(`${API_URL}/users/`, payload)
  return res.data;
})
export const loginUser = createAsyncThunk<User, UserLoginFormValues>('users/loginUser', async (payload, thunkAPI) =>{
...
export const {addItemToCart, removeFromCart, decreaseQuantity, increaseQuantity, logOut} = userSlice.actions
