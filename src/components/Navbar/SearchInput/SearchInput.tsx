...
const SearchInput = React.forwardRef<HTMLInputElement, SearchInputProps>((
  { value, onChange, products, searchUI, variant, size, animation, className, ...props },
  ref
) => {
  ...
  return (
    <div className="relative">
      ...
      {isExpanded && (
        <div className={`search__products absolute w-full bg-white rounded-b-md shadow-md z-10 transition-opacity duration-300 ease-in-out max-h-[295px] overflow-y-auto ${isAnimating ? 'opacity-100' : 'opacity-0'}`}>
          {products.length > 0 && products.map((product, index) => (
            <Link
              key={product.id}
              to={`/product/${product.id}`}
              className={`flex gap-5 items-center p-1 hover:shadow-customPink transition-shadow duration-300 ${index !== products.length - 1 ? 'border-b' : ''}`}
            >
              <div className="w-10 h-10">
                <img className="w-full h-full rounded-md" src={product.images[0].replace(/[\[\]\"]/g, '')} alt="Product image" />
              </div>
